class PokemonDataFetchLogic {
  ...
  dataFetchSecondEvolutionDataIsAvailable(
      data, String layer1, String layer2, String pokemonName) {
    final List obtainedData = [];
    ...
    for (int i = 0; i < pokemonQuantity; i++) {
      ...
      String obtainedUrl =
          pokemonsUrl[i].toString().substring(42, (pokemonsUrl[i].length - 1));
  ...
  dataFetchThirdEvolutionDataIsAvailable(
      data, String layer1, String layer2, String layer3) {
    final List obtainedData = [];
    ...
    for (int i = 0; i < pokemonQuantity; i++) {
      ...
      if ((data2?.isEmpty ?? true)) {
        ...
        // E.g https://pokeapi.co/api/v2/pokemon-species/
        // Remove first 42 char and Remove last char "/" from URL to obtain a number
        String obtainedUrl = pokemonsUrl[i]
            .toString()
            .substring(42, (pokemonsUrl[i].length - 1));
        int pokemonId = int.parse(obtainedUrl);
        // Filter Pokemon up to the Limit 721
        if (pokemonId > 721) {
          //Do Nothing
  ...
  dataFetchPokemonEvolution(data) {
    ...
    // If a value is found - a evolution chain is available
      ...
      if (pokemonId > 721) {
        ...
        List pokemonName2 = dataFetchSecondEvolutionDataIsAvailable(
            data['evolves_to'], 'species', "name", pokemonName1);
        pokemonEvolutionChain.add(pokemonName2);

        // Check and Add 3rd Evolution(s) if available
        List pokemonName3 = dataFetchThirdEvolutionDataIsAvailable(
            data['evolves_to'], 'evolves_to', 'species', 'name');
